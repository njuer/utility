...
public class ShiShiCaiScheduler {

	private static Logger logger = Logger.getLogger(ShiShiCaiScheduler.class
			.getName());
	...
	public static void main(String[] args) {
		...
		JobDetail job = JobBuilder.newJob(ShiShiCaiJob.class)
				.withIdentity("jxsscJob", "groupSSC").build();
		JobDataMap dataMap = job.getJobDataMap();
		dataMap.put(ShiShiCaiJob.PARAM_HTTPSIMULATION, hs);
		dataMap.put(ShiShiCaiJob.PARAM_NVPS, nvps);
		dataMap.put(ShiShiCaiJob.PARAM_MAIL_SESSION, session);
		dataMap.put(ShiShiCaiJob.PARAM_MAIL_HOST, host);
		dataMap.put(ShiShiCaiJob.PARAM_MAIL_USER, user);
		dataMap.put(ShiShiCaiJob.PARAM_MAIL_PWD, pwd);
		dataMap.put(ShiShiCaiJob.PARAM_MAIL_TO, to);
		
		//每天8-22点，每时段2分开始执行，间隔为10分钟
